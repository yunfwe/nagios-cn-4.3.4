...
int main(void) {
	...
	if(is_authorized_for_read_only(&current_authdata) == TRUE) {
		printf("<P><DIV CLASS='errorMessage'>看来您没有权限提交您请求的命令。。。</DIV></P>\n");
	...
	if(command_type == CMD_NONE) {
		if(content_type == WML_CONTENT)
			printf("<p>错误：没有指定命令！</p>\n");
		else
			printf("<P><DIV CLASS='errorMessage'>错误：没有指定命令！</DIV></P>\n");
		}
...
void request_command_data(int cmd) {
	...
	switch(cmd) {
		...
		case CMD_ADD_SVC_COMMENT:
			printf("添加一个%s注释", (cmd == CMD_ADD_HOST_COMMENT) ? "主机" : "服务");
			break;

		case CMD_DEL_HOST_COMMENT:
		case CMD_DEL_SVC_COMMENT:
			printf("删除一个%s注释", (cmd == CMD_DEL_HOST_COMMENT) ? "主机" : "服务");
			break;

		case CMD_DELAY_HOST_NOTIFICATION:
		case CMD_DELAY_SVC_NOTIFICATION:
			printf("延迟一个%s通知", (cmd == CMD_DELAY_HOST_NOTIFICATION) ? "主机" : "服务");
			break;
		...
		case CMD_DISABLE_SVC_CHECK:
			printf("%s对特定服务的主动检测", (cmd == CMD_ENABLE_SVC_CHECK) ? "启用" : "禁用");
			break;
		...
		case CMD_DISABLE_HOST_SVC_CHECKS:
			printf("%s主机上所有服务的主动检测", (cmd == CMD_ENABLE_HOST_SVC_CHECKS) ? "启用" : "禁用");
			break;

		case CMD_SCHEDULE_HOST_SVC_CHECKS:
			printf("安排对主机的所有服务的检测");
			break;

		case CMD_DEL_ALL_HOST_COMMENTS:
		case CMD_DEL_ALL_SVC_COMMENTS:
			printf("删除这个%s的所有注释", (cmd == CMD_DEL_ALL_HOST_COMMENTS) ? "主机" : "服务");
			break;

		case CMD_ENABLE_SVC_NOTIFICATIONS:
		case CMD_DISABLE_SVC_NOTIFICATIONS:
			printf("%s这个服务的通知", (cmd == CMD_ENABLE_SVC_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOST_NOTIFICATIONS:
		case CMD_DISABLE_HOST_NOTIFICATIONS:
			printf("%s这个主机的通知", (cmd == CMD_ENABLE_HOST_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_ALL_NOTIFICATIONS_BEYOND_HOST:
		case CMD_DISABLE_ALL_NOTIFICATIONS_BEYOND_HOST:
			printf("%s这个主机以外所有主机和服务的通知", (cmd == CMD_ENABLE_ALL_NOTIFICATIONS_BEYOND_HOST) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOST_SVC_NOTIFICATIONS:
		case CMD_DISABLE_HOST_SVC_NOTIFICATIONS:
			printf("%s这个主机上所有服务的通知", (cmd == CMD_ENABLE_HOST_SVC_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ACKNOWLEDGE_HOST_PROBLEM:
		case CMD_ACKNOWLEDGE_SVC_PROBLEM:
			printf("公认这个%s的问题", (cmd == CMD_ACKNOWLEDGE_HOST_PROBLEM) ? "主机" : "服务");
			break;

		case CMD_START_EXECUTING_SVC_CHECKS:
		case CMD_STOP_EXECUTING_SVC_CHECKS:
			printf("%s执行主动服务检测", (cmd == CMD_START_EXECUTING_SVC_CHECKS) ? "开始" : "停止");
			break;

		case CMD_START_ACCEPTING_PASSIVE_SVC_CHECKS:
		case CMD_STOP_ACCEPTING_PASSIVE_SVC_CHECKS:
			printf("%s接受被动服务检测", (cmd == CMD_START_ACCEPTING_PASSIVE_SVC_CHECKS) ? "开始" : "停止");
			break;

		case CMD_ENABLE_PASSIVE_SVC_CHECKS:
		case CMD_DISABLE_PASSIVE_SVC_CHECKS:
			printf("%s接受特定服务的被动服务检查", (cmd == CMD_ENABLE_PASSIVE_SVC_CHECKS) ? "开始" : "停止");
			break;

		case CMD_ENABLE_EVENT_HANDLERS:
		case CMD_DISABLE_EVENT_HANDLERS:
			printf("%s事件处理", (cmd == CMD_ENABLE_EVENT_HANDLERS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOST_EVENT_HANDLER:
		case CMD_DISABLE_HOST_EVENT_HANDLER:
			printf("%s特定主机的事件处理", (cmd == CMD_ENABLE_HOST_EVENT_HANDLER) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_SVC_EVENT_HANDLER:
		case CMD_DISABLE_SVC_EVENT_HANDLER:
			printf("%s特定服务的事件处理", (cmd == CMD_ENABLE_SVC_EVENT_HANDLER) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOST_CHECK:
		case CMD_DISABLE_HOST_CHECK:
			printf("%s特定主机的服务检测", (cmd == CMD_ENABLE_HOST_CHECK) ? "启用" : "禁用");
			break;

		case CMD_STOP_OBSESSING_OVER_SVC_CHECKS:
		case CMD_START_OBSESSING_OVER_SVC_CHECKS:
			printf("%s专注于服务检测", (cmd == CMD_STOP_OBSESSING_OVER_SVC_CHECKS) ? "停止" : "开始");
			break;

		case CMD_REMOVE_HOST_ACKNOWLEDGEMENT:
		case CMD_REMOVE_SVC_ACKNOWLEDGEMENT:
			printf("移除这个%s的公认", (cmd == CMD_REMOVE_HOST_ACKNOWLEDGEMENT) ? "主机" : "服务");
			break;

		case CMD_SCHEDULE_HOST_DOWNTIME:
		case CMD_SCHEDULE_SVC_DOWNTIME:
			printf("安排这个特定%s的停机时间", (cmd == CMD_SCHEDULE_HOST_DOWNTIME) ? "主机" : "服务");
			break;

		case CMD_SCHEDULE_HOST_SVC_DOWNTIME:
			printf("安排特定主机所有服务的停机时间");
			break;

		case CMD_PROCESS_HOST_CHECK_RESULT:
		case CMD_PROCESS_SERVICE_CHECK_RESULT:
			printf("提交特定%s的被动检测结果", (cmd == CMD_PROCESS_HOST_CHECK_RESULT) ? "主机" : "服务");
			break;

		case CMD_ENABLE_HOST_FLAP_DETECTION:
		case CMD_DISABLE_HOST_FLAP_DETECTION:
			printf("%s特定主机的波动检测", (cmd == CMD_ENABLE_HOST_FLAP_DETECTION) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_SVC_FLAP_DETECTION:
		case CMD_DISABLE_SVC_FLAP_DETECTION:
			printf("%s特定服务的波动检测", (cmd == CMD_ENABLE_SVC_FLAP_DETECTION) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_FLAP_DETECTION:
		case CMD_DISABLE_FLAP_DETECTION:
			printf("%s主机和服务的波动检测", (cmd == CMD_ENABLE_FLAP_DETECTION) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOSTGROUP_SVC_NOTIFICATIONS:
		case CMD_DISABLE_HOSTGROUP_SVC_NOTIFICATIONS:
			printf("%s特定主机组的所有服务通知", (cmd == CMD_ENABLE_HOSTGROUP_SVC_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOSTGROUP_HOST_NOTIFICATIONS:
		case CMD_DISABLE_HOSTGROUP_HOST_NOTIFICATIONS:
			printf("%s特定主机组的所有主机通知", (cmd == CMD_ENABLE_HOSTGROUP_HOST_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_HOSTGROUP_SVC_CHECKS:
		case CMD_DISABLE_HOSTGROUP_SVC_CHECKS:
			printf("%s特定主机组所有服务的主动检测", (cmd == CMD_ENABLE_HOSTGROUP_SVC_CHECKS) ? "启用" : "禁用");
			break;

		case CMD_DEL_HOST_DOWNTIME:
		case CMD_DEL_SVC_DOWNTIME:
			printf("取消安排这个特定%s的停机时间", (cmd == CMD_DEL_HOST_DOWNTIME) ? "主机" : "服务");
			break;
		...
		case CMD_SCHEDULE_HOSTGROUP_HOST_DOWNTIME:
			printf("安排特定主机组所有主机的停机时间");
			break;

		case CMD_SCHEDULE_HOSTGROUP_SVC_DOWNTIME:
			printf("安排特定主机组所有服务的停机时间");
			break;

		case CMD_START_EXECUTING_HOST_CHECKS:
		case CMD_STOP_EXECUTING_HOST_CHECKS:
			printf("%s执行主机检测", (cmd == CMD_START_EXECUTING_HOST_CHECKS) ? "开始" : "停止");
			break;

		case CMD_START_ACCEPTING_PASSIVE_HOST_CHECKS:
		case CMD_STOP_ACCEPTING_PASSIVE_HOST_CHECKS:
			printf("%s接受这个主机的被动检测", (cmd == CMD_START_ACCEPTING_PASSIVE_HOST_CHECKS) ? "开始" : "停止");
			break;

		case CMD_ENABLE_PASSIVE_HOST_CHECKS:
		case CMD_DISABLE_PASSIVE_HOST_CHECKS:
			printf("%s接受特定主机的被动检测", (cmd == CMD_ENABLE_PASSIVE_HOST_CHECKS) ? "开始" : "停止");
			break;

		case CMD_START_OBSESSING_OVER_HOST_CHECKS:
		case CMD_STOP_OBSESSING_OVER_HOST_CHECKS:
			printf("%s专注于这个主机的检测", (cmd == CMD_START_OBSESSING_OVER_HOST_CHECKS) ? "开始" : "停止");
			break;
		...
		case CMD_STOP_OBSESSING_OVER_SVC:
			printf("%s专注于特定的服务", (cmd == CMD_START_OBSESSING_OVER_SVC) ? "开始" : "停止");
			break;

		case CMD_START_OBSESSING_OVER_HOST:
		case CMD_STOP_OBSESSING_OVER_HOST:
			printf("%s专注于特定的主机", (cmd == CMD_START_OBSESSING_OVER_HOST) ? "开始" : "停止");
			break;

		case CMD_ENABLE_SERVICEGROUP_SVC_NOTIFICATIONS:
		case CMD_DISABLE_SERVICEGROUP_SVC_NOTIFICATIONS:
			printf("%s特定服务组所有服务的通知", (cmd == CMD_ENABLE_SERVICEGROUP_SVC_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_SERVICEGROUP_HOST_NOTIFICATIONS:
		case CMD_DISABLE_SERVICEGROUP_HOST_NOTIFICATIONS:
			printf("%s特定服务组所有主机的通知", (cmd == CMD_ENABLE_SERVICEGROUP_HOST_NOTIFICATIONS) ? "启用" : "禁用");
			break;

		case CMD_ENABLE_SERVICEGROUP_SVC_CHECKS:
		case CMD_DISABLE_SERVICEGROUP_SVC_CHECKS:
			printf("%s主动检测特定服务组所有服务", (cmd == CMD_ENABLE_SERVICEGROUP_SVC_CHECKS) ? "启用" : "禁用");
			break;

		case CMD_SCHEDULE_SERVICEGROUP_HOST_DOWNTIME:
			printf("安排特定服务组所有主机的停机时间");
			break;

		case CMD_SCHEDULE_SERVICEGROUP_SVC_DOWNTIME:
			printf("安排特定服务组所有服务的停机时间");
			break;

		case CMD_CLEAR_HOST_FLAPPING_STATE:
		case CMD_CLEAR_SVC_FLAPPING_STATE:
			printf("清理这个%s的状态波动记录", (cmd == CMD_CLEAR_HOST_FLAPPING_STATE) ? "主机" : "服务");
			break;

		case CMD_SEND_CUSTOM_HOST_NOTIFICATION:
		case CMD_SEND_CUSTOM_SVC_NOTIFICATION:
			printf("发送自定义%s通知", (cmd == CMD_SEND_CUSTOM_HOST_NOTIFICATION) ? "主机" : "服务");
			break;
    ...
    printf("<table class='mytab' lay-skin='nob' lay-size='lsm' style='width:600px;'>");
	...
	switch(cmd) {
		...
		case CMD_ACKNOWLEDGE_HOST_PROBLEM:
			...
			if(cmd == CMD_ACKNOWLEDGE_HOST_PROBLEM) {
				printf("<tr><td CLASS='optBoxItem'>凝滞公认：</td><td><b>");
			...
			printf("<tr><td CLASS='optBoxItem'>持续的%s:</td><td><b>", (cmd == CMD_ACKNOWLEDGE_HOST_PROBLEM) ? " 注释" : "");
		...
		case CMD_ACKNOWLEDGE_SVC_PROBLEM:
			...
			printf("<tr><td CLASS='optBoxItem'>持续的%s:</td><td><b>", (cmd == CMD_ACKNOWLEDGE_SVC_PROBLEM) ? " 注释" : "");
		...
		case CMD_DELAY_HOST_NOTIFICATION:
			...
			printf("<INPUT TYPE='TEXT' NAME='host' VALUE='%s'>", escape_string(host_name));
			printf("</b></td></tr>\n");
			printf("<tr><td CLASS='optBoxRequiredItem'>通知延迟 （从现在开始几分钟）：</td><td><b>");
		...
		case CMD_SCHEDULE_HOST_SVC_CHECKS:
			...
			printf("<tr><td CLASS='optBoxRequiredItem'>检测时间：</td><td><b>");
			printf("<INPUT TYPE='TEXT' NAME='start_time' VALUE='%s'>", buffer);
			printf("</b></td></tr>\n");
			printf("<tr><td CLASS='optBoxItem'>强制检测：</td><td><b>");
		...
		case CMD_CLEAR_HOST_FLAPPING_STATE:
			...
			if(cmd == CMD_ENABLE_HOST_SVC_CHECKS || cmd == CMD_DISABLE_HOST_SVC_CHECKS || cmd == CMD_ENABLE_HOST_SVC_NOTIFICATIONS || cmd == CMD_DISABLE_HOST_SVC_NOTIFICATIONS) {
				printf("<tr><td CLASS='optBoxItem'>也%s此主机：</td><td><b>", (cmd == CMD_ENABLE_HOST_SVC_CHECKS || cmd == CMD_ENABLE_HOST_SVC_NOTIFICATIONS) ? "启用" : "禁用");
				printf("<INPUT TYPE='checkbox' NAME='ahas'>");
				printf("</b></td></tr>\n");
				}
			if(cmd == CMD_ENABLE_HOST_NOTIFICATIONS || cmd == CMD_DISABLE_HOST_NOTIFICATIONS) {
				printf("<tr><td CLASS='optBoxItem'>同时%s子主机通知：</td><td><b>", (cmd == CMD_ENABLE_HOST_NOTIFICATIONS) ? "启用" : "禁用");
		...
		case CMD_SCHEDULE_SVC_DOWNTIME:
			...
			printf("<select name='trigger' style='width:50px;'>\n");
			...
			printf("<OPTION VALUE=1>固定的\n");
			printf("<OPTION VALUE=0>弹性的\n");
			printf("</SELECT>\n");
			printf("</b></td></tr>\n");

			printf("<tr><td CLASS='optBoxItem'>如果弹性的, 持续：</td><td>");
			printf("<table border=0><tr>\n");
			printf("<td align=right><INPUT TYPE='TEXT' NAME='hours' VALUE='2' SIZE=2 MAXLENGTH=2></td>\n");
			printf("<td align=left>时</td>\n");
			...
			if(cmd == CMD_SCHEDULE_HOST_DOWNTIME) {
				...
				printf("<option value='0'>对子主机不做任何操作\n");
				printf("<option value='1'>对所有子主机安排触发停机时间\n");
				printf("<option value='2'>对所有子主机安排非触发停机时间\n\n");
				printf("</SELECT>\n");
				printf("</b></td></tr>\n");
				}
		...
		case CMD_ENABLE_HOSTGROUP_SVC_CHECKS:
		case CMD_DISABLE_HOSTGROUP_SVC_CHECKS:
			printf("<tr><td CLASS='optBoxRequiredItem'>主机组名称：</td><td><b>");
		...
		case CMD_ENABLE_SERVICEGROUP_SVC_CHECKS:
		case CMD_DISABLE_SERVICEGROUP_SVC_CHECKS:
			printf("<tr><td CLASS='optBoxRequiredItem'>服务组名称：</td><td><b>");
		...
		case CMD_DEL_HOST_DOWNTIME:
		case CMD_DEL_SVC_DOWNTIME:
			printf("<tr><td CLASS='optBoxRequiredItem'>安排停机时间ID:</td><td><b>");
		...
		case CMD_SCHEDULE_SERVICEGROUP_SVC_DOWNTIME:
			...
			printf("<SELECT NAME='fixed'>");
			printf("<OPTION VALUE=1>固定的\n");
			printf("<OPTION VALUE=0>灵活的\n");
			printf("</SELECT>\n");
			printf("</b></td></tr>\n");

			printf("<tr><td CLASS='optBoxItem'>如果灵活的, 持续：</td><td>");
			printf("<table border=0><tr>\n");
			printf("<td align=right><INPUT TYPE='TEXT' NAME='hours' VALUE='2' SIZE=2 MAXLENGTH=2></td>\n");
			printf("<td align=left>时</td>\n");
			printf("<td align=right><INPUT TYPE='TEXT' NAME='minutes' VALUE='0' SIZE=2 MAXLENGTH=2></td>\n");
			...
			if(cmd == CMD_SCHEDULE_HOSTGROUP_SVC_DOWNTIME || cmd == CMD_SCHEDULE_SERVICEGROUP_SVC_DOWNTIME) {
				printf("<tr><td CLASS='optBoxItem'>再次主机安排停机时间：</td><td><b>");
				printf("<INPUT TYPE='checkbox' NAME='ahas'>");
				printf("</b></td></tr>\n");
				}
			break;
		...
		case CMD_SEND_CUSTOM_SVC_NOTIFICATION:
			...
			printf("<tr><td CLASS='optBoxItem'>强制的：</td><td><b>");
			printf("<INPUT TYPE='checkbox' NAME='force_notification' ");
			printf("</b></td></tr>\n");

			printf("<tr><td CLASS='optBoxItem'>广播：</td><td><b>");
		...
		default:
			printf("<tr><td CLASS='optBoxItem'>这不应该发生。。。</td><td></td></tr>\n");
		}


	printf("<tr><td CLASS='optBoxItem' COLSPAN=2></td></tr>\n");
	printf("<tr><td CLASS='optBoxItem'></td><td CLASS='optBoxItem'><INPUT TYPE='submit' NAME='btnSubmit' VALUE='提交' class='my-btn my-btn-primary'> <INPUT TYPE='reset' VALUE='重置' class='my-btn my-btn-primary'></td></tr>\n");
	...
	printf("<P><DIV CLASS='infoMessage'>在输入命令之前，请先输入所有必需的信息。<br>必填字段用红色标记。<br>未能提供所有所需的值将导致错误发生。</DIV></P>");
