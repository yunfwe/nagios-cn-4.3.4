...
int main(void) {
	...
	if(display_header == TRUE) {
		...
		if(display_type == DISPLAY_SERVICES)
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "服务报警历史");
		else if(show_all_hosts == TRUE)
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "报警历史");
		else
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "主机报警历史");
		...
		if(display_type == DISPLAY_HOSTS) {
			printf("<A HREF='%s?host=%s'>查看%s的状态详细信息</A><BR />\n", STATUS_CGI, (show_all_hosts == TRUE) ? "all" : url_encode(host_name), (show_all_hosts == TRUE) ? "所有主机" : "这个主机");
			printf("<A HREF='%s?host=%s'>查看%s的通知</A><BR />\n", NOTIFICATIONS_CGI, (show_all_hosts == TRUE) ? "all" : url_encode(host_name), (show_all_hosts == TRUE) ? "所有主机" : "这个主机");
#ifdef USE_TRENDS
			if(show_all_hosts == FALSE)
				printf("<A HREF='%s?host=%s'>查看此主机的趋势</A>\n", TRENDS_CGI, url_encode(host_name));
#endif
			}
		else {
			printf("<A HREF='%s?host=%s&", NOTIFICATIONS_CGI, url_encode(host_name));
			printf("service=%s'>查看此服务的通知</A><BR />\n", url_encode(svc_description));
#ifdef USE_TRENDS
			printf("<A HREF='%s?host=%s&", TRENDS_CGI, url_encode(host_name));
			printf("service=%s'>查看此服务的趋势</A><BR />\n", url_encode(svc_description));
#endif
			printf("<A HREF='%s?host=%s'>查看此主机的历史记录</A>\n", HISTORY_CGI, url_encode(host_name));
			}
		printf("</TD></TR>\n");
		printf("</TABLE>\n");
		...
		if(display_type == DISPLAY_SERVICES)
			printf("在主机 '%s' 上的 '%s' 服务", host_name,svc_description);
		else if(show_all_hosts == TRUE)
			printf("所有主机和服务");
		else
		...
		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'>状态类型选择：</td>\n");
		printf("</tr>\n");
		...
		if(display_type == DISPLAY_HOSTS)
			printf("<td align=left CLASS='optBoxItem'>%s主机的历史详细级别：", (show_all_hosts == TRUE) ? "所有" : "这个");
		else
			printf("服务");
		printf("</td>\n");
		printf("</tr>\n")
		...
		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='noflapping' %s> 隐藏状态波动报警</td>", (display_flapping_alerts == FALSE) ? "checked" : "");
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='nodowntime' %s> 隐藏停机时间报警</td>", (display_downtime_alerts == FALSE) ? "checked" : "");
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='nosystem' %s> 隐藏处理消息</td>", (display_system_messages == FALSE) ? "checked" : "");
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='oldestfirst' %s> 先看旧信息</td>", (use_lifo == FALSE) ? "checked" : "");
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'><input type='submit' value='提交'></td>\n");
		printf("</tr>\n");
...
void get_history(void) {
	...
	if(use_lifo == TRUE) {
		...
		if(result != LIFO_OK) {
			if(result == LIFO_ERROR_MEMORY) {
				printf("<P><DIV CLASS='warningMessage'>内存不足，无法反转日志文件——按自然顺序显示历史。。。</DIV></P>\n");
				}
			else if(result == LIFO_ERROR_FILE) {
				printf("<HR><P><DIV CLASS='errorMessage'>错误：无法打开读取日志文件'%s'！</DIV></P><HR>", log_file_to_use);
				return;
				}
			use_lifo = FALSE;
			}
	...
	while(1) {
		...
		/* service flapping alerts */
			...
			if(strstr(input, ";STARTED;"))
				strncpy(image_alt, "服务状态开始波动", sizeof(image_alt));
			else if(strstr(input, ";STOPPED;"))
				strncpy(image_alt, "服务状态停止波动", sizeof(image_alt));
			else if(strstr(input, ";DISABLED;"))
				strncpy(image_alt, "服务状态波动检测已禁用", sizeof(image_alt));
			}
		...
		/* service downtime alerts */
			...
			if(strstr(input, ";STARTED;"))
				strncpy(image_alt, "服务进入预定停机时间", sizeof(image_alt));
			else if(strstr(input, ";STOPPED;"))
				strncpy(image_alt, "服务退出预定停机时间", sizeof(image_alt));
			else if(strstr(input, ";CANCELLED;"))
				strncpy(image_alt, "服务安排停机时间已被取消", sizeof(image_alt));
			}
		...
		/* host flapping alerts */
			...
			if(strstr(input, ";STARTED;"))
				strncpy(image_alt, "主机状态开始波动", sizeof(image_alt));
			else if(strstr(input, ";STOPPED;"))
				strncpy(image_alt, "主机状态停止波动", sizeof(image_alt));
			else if(strstr(input, ";DISABLED;"))
				strncpy(image_alt, "主机状态波动检测已禁用", sizeof(image_alt));
			}
		...
		/* host downtime alerts */
			...
			if(strstr(input, ";STARTED;"))
				strncpy(image_alt, "主机进入预定停机时间", sizeof(image_alt));
			else if(strstr(input, ";STOPPED;"))
				strncpy(image_alt, "主机退出预定停机时间", sizeof(image_alt));
			else if(strstr(input, ";CANCELLED;"))
				strncpy(image_alt, "主机安排停机时间已被取消", sizeof(image_alt));
			}
		...
		if(strcmp(image, "")) {
			...
			if(system_message == TRUE)
				...
				if(history_type == HOST_HISTORY || history_type == SERVICE_HISTORY) {
					...
					snprintf(match1, sizeof( match1),
							" 主机状态波动报警: %s;", host_name);
					snprintf(match2, sizeof( match2),
							" 服务状态波动报警: %s;", host_name);
					}
				else if(history_type == HOST_DOWNTIME_HISTORY || history_type == SERVICE_DOWNTIME_HISTORY) {
					snprintf(match1, sizeof( match1),
							" 主机停机时间报警: %s;", host_name);
					snprintf(match2, sizeof( match2),
							" 服务停机时间报警: %s;", host_name);
					}
				...
				if(history_type == SERVICE_HISTORY)
					snprintf(match1, sizeof( match1), " 服务报警: %s;%s;", host_name, svc_description);
				else if(history_type == SERVICE_FLAPPING_HISTORY)
					snprintf(match1, sizeof( match1), " 服务状态波动报警: %s;%s;", host_name, svc_description);
				else if(history_type == SERVICE_DOWNTIME_HISTORY)
					snprintf(match1, sizeof( match1), " 服务停机时间报警: %s;%s;", host_name, svc_description);
	...
	if(found_line == FALSE) {
		...
		if(display_type == DISPLAY_HOSTS)
			printf("%s", (show_all_hosts == TRUE) ? "" : "这个主机的");
		else
			printf("这个服务的 ");
		printf("%s日志文件记录</DIV></P>", (log_archive == 0) ? "当前" : "归档");
		}
