...
void display_info_table(const char *title, int refresh, authdata *current_authdata) {
	...
	time(&current_time);
	get_time_string(&current_time, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
	//printf("<blockquote  class='layui-elem-quote layui-quote-nm'>");	
    printf("<blockquote class='myqou'>");
	printf("最近一次更新： %s<BR>\n", date_time);
	if(refresh == TRUE)
		printf("每 %d 秒更新一次<br>\n", refresh_rate);
	...
	if(nagios_process_state != STATE_OK)
		printf("<DIV CLASS='infoBoxBadProcStatus'>警告：监控进程没有运行！<BR>点击<A HREF='%s?type=%d'>这里</A>查看更多信息。</DIV>", EXTINFO_CGI, DISPLAY_PROCESS_INFO);

	if(result == ERROR)
		printf("<DIV CLASS='infoBoxBadProcStatus'>警告：无法读取程序状态信息！</DIV>");

	else {
		if(enable_notifications == FALSE)
			printf("<DIV CLASS='infoBoxBadProcStatus'>- 通知已禁用</DIV>");

		if(execute_service_checks == FALSE)
			printf("<DIV CLASS='infoBoxBadProcStatus'>- 服务检查已禁用</DIV>");
		}
...
void display_nav_table(char *url, int archive) {
	...
	if(log_rotation_method != LOG_ROTATION_NONE) {
		...
		if(archive == 0) {
			printf("最新归档<br>");
			printf("<a href='%sarchive=1'><img src='%s%s' border=0 alt='最新归档' title='Latest Archive'></a>", url, url_images_path, LEFT_ARROW_ICON);
			}
		else {
			printf("早期的归档<br>");
			printf("<a href='%sarchive=%d'><img src='%s%s' border=0 alt='早期的归档' title='Earlier Archive'></a>", url, archive + 1, url_images_path, LEFT_ARROW_ICON);
			}
		printf("</td>\n");
...
void cgi_config_file_error(const char *config_file) {
	...
	printf("<div class='mytops' style='margin-top:15%%;margin-left:25%%;'>\n");
	printf("<H1>注意！</H1>\n");

	printf("<P><STRONG><FONT COLOR='RED'>错误: 不能打开CGI配置文件 '%s'！</FONT></STRONG></P>\n", config_file);

	printf("<P>\n");
	printf("为了解决这个错误，您应该检查一些东西：\n");
	printf("</P>\n");

	printf("<P>\n");
	printf("<OL>\n");

	printf("<LI>确保在适当位置安装了CGI配置文件。\n");
	printf("<LI>确保您的Web服务器运行时具有读取CGI配置文件的权限。\n");
...
void main_config_file_error(const char *config_file) {
	...
	printf("<P><STRONG><FONT COLOR='RED'>错误: 不能读取主配置文件 '%s'！</FONT></STRONG></P>\n", config_file);
	printf("</div>\n");
	// printf("<P>\n");
...
void object_data_error(void) {
	...
	printf("<P><STRONG><FONT COLOR='RED'>错误：无法读取对象的配置数据！</FONT></STRONG></P>\n");
	printf("</div>\n");
	// printf("<P>\n");
	// printf("Here are some things you should check in order to resolve this error:\n");
	// printf("</P>\n");
...
void status_data_error(void) {
	...
	printf("<P><STRONG><FONT COLOR='RED'>错误：无法读取主机和服务状态信息！</FONT></STRONG></P>\n");

	printf("<P>\n");
	printf("请确定已经启动Nagios服务 或者检查Nagios服务状态是否正常\n");
	printf("</P>\n");
	printf("</div>\n");
